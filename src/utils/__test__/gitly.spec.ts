...
describe('gitly', () => {
  const destination = join(__dirname, 'output', 'gitly', 'example')
  const options = {
    temp: join(__dirname, 'output', 'gitly', '.gitcopy'),
  }

  beforeEach(async () => {
    rm('-rf', join(__dirname, 'output', 'gitly', '.gitcopy'))
  })
  afterEach(async () => {
    rm('-rf', destination)
  })
  ...
  it('should clone the repository using git', async () => {
    const result = await gitly('lukeed/gittar', destination, { ...options, backend: 'git' })
    expect(await exists(result[0])).toBeDefined()
    expect(await exists(result[1])).toBeDefined()
  })
});